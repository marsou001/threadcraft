...
export async function deleteAllGeneratedContentForUser(id: number) {
  console.log("Deleting content generated by user", id);

  const userContent = await db
    .select({ id: GeneratedContent.id, socialMedia: GeneratedContent.socialMedia })
    .from(GeneratedContent)
    .where(eq(GeneratedContent.user, id))
    .execute();
  
  await Promise.all(userContent.map(async (content) => {
    switch(content.socialMedia) {
      case "X":
        await deleteXSettings(content.id);
        break;
      case "Instagram":
        await deleteInstagramSettings(content.id);
        break;
      case "LinkedIn":
        await deleteLinkedInSettings(content.id);
        break;
    }
...
async function deleteGeneratedContent(id: number) {
  await db
...
async function deleteXSettings(generatedContentId: number) {
  await db
    .delete(XSettings)
    .where(eq(XSettings.generatedContentId, generatedContentId))
    .execute()
}
async function deleteInstagramSettings(generatedContentId: number) {
  await db
    .delete(InstagramSettings)
    .where(eq(InstagramSettings.generatedContentId, generatedContentId))
    .execute()
}

async function deleteLinkedInSettings(generatedContentId: number) {
  await db
    .delete(LinkedInSettings)
    .where(eq(LinkedInSettings.generatedContentId, generatedContentId))
    .execute()
}